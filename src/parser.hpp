...
inline VMInstruction make_open_write(int32_t cursor_id, const char* table_name, int32_t index_column = 0) {
...
// Transactions
inline VMInstruction make_begin() {
    return {OP_Begin, 0, 0, 0, nullptr, 0};
}

inline VMInstruction make_commit() {
    return {OP_Commit, 0, 0, 0, nullptr, 0};
}
...
inline VMInstruction make_flush() {
    return {Op_Flush, 0, 0, 0, nullptr, 0};
}
