...
static void
set_child(BPlusTree &tree, BTreeNode *node, uint32_t child_index,
	  uint32_t node_index)
{
	if (!node || node->is_leaf) {
		return;
...
static FindResult
find_containing_node(BPlusTree &tree, BTreeNode *node, const uint8_t *key,
		     BtCursor *cursor)
{
