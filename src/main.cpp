...
uint32_t weighted_rand_op() {
    // Weights: 30% create, 30% write, 30% delete, 5% rollback, 5% commit
    int r = std::rand() % 100;
    if (r < 30) return 0; // Create
    if (r < 60) return 1; // Write
    if (r < 90) return 2; // Delete
    if (r < 95) return 3; // Rollback
    return 4; // Commit
}
...
void test_pager_stress() {
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        if (operation == 0) { // Create new page
            ...
            PagerMeta new_stats = pager_get_stats();
            std::cout << "Stats: free_pages=" << new_stats.free_pages << ", total_pages=" << new_stats.total_pages << "\n";
        }
        else if (operation == 1 && (committed_pages.size + transaction_pages.size > 0)) { // Write to random page
            if (!in_transaction) {
                stats = pager_get_stats();
                before_hash = hash_file(DB);
                pager_begin_transaction();
            ...
            uint32_t page_id = index < committed_pages.size ? committed_pages.data[index] : transaction_pages.data[index - committed_pages.size];
        ...
        else if (operation == 2 && (committed_pages.size + transaction_pages.size > 0)) { // Delete random page
            if (!in_transaction) {
                stats = pager_get_stats();
                before_hash = hash_file(DB);
                pager_begin_transaction();
        ...
        else if (operation == 3 && in_transaction) { // Rollback transaction
            pager_rollback();
            ...
            PagerMeta new_stats = pager_get_stats();
            std::cout << "Stats after rollback: free_pages=" << new_stats.free_pages << ", total_pages=" << new_stats.total_pages << "\n";
            ...
            assert(new_stats.free_pages == stats.free_pages && new_stats.total_pages == stats.total_pages && "Stats not restored after rollback");
        }
        else if (operation == 4 && in_transaction && made_changes) { // Commit transaction
            pager_commit();
            in_transaction = false;
            made_changes = false;
            ...
            array_clear(&transaction_pages);
            PagerMeta new_stats = pager_get_stats();
            std::cout << "Stats after commit: free_pages=" << new_stats.free_pages << ", total_pages=" << new_stats.total_pages << "\n";
            uint64_t after_hash = hash_file(DB);
    ...
    PagerMeta final_stats = pager_get_stats();
