...
{
	return get_internal_record_data(tree, node) +
	       (index * tree.record_size);
}
...
binary_search(BTree &tree, BTreeNode *node, const uint8_t *key)
{
	...
	while (left < right) {
		...
		int cmp_result =
		    cmp(tree.node_key_size, get_key_at(tree, node, mid), key);
...
BTree
btree_create(DataType key, uint32_t record_size, TreeType tree_type)
{
	BTree tree = {0};
	...
	if (tree_type == BTREE) {
		...
		uint32_t max_keys = (USABLE_SPACE - child_ptr_size) /
...
static void
set_child(BTree &tree, BTreeNode *node, uint32_t child_index,
	  uint32_t node_index)
{
	if (!node || node->is_leaf) {
		return;
	...
	if (node_index != 0) {
		BTreeNode *child_node =
		    static_cast<BTreeNode *>(pager_get(node_index));
		if (child_node) {
...
split(BTree &tree, BTreeNode *node)
{
	...
	if (!parent) {
		...
		if (tree.tree_type == BTREE) {
			uint8_t *parent_records =
			    get_internal_record_data(tree, parent);
			memcpy(parent_records +
				   (parent_index + 1) * tree.record_size,
			       parent_records + parent_index * tree.record_size,
			       (parent->num_keys - parent_index) *
				   tree.record_size);
		}
	}
	...
	if (tree.tree_type == BTREE) {
		...
		memcpy(parent_records + parent_index * tree.record_size,
		       source_records + split_index * tree.record_size,
		       tree.record_size);
	}
	...
	if (node->is_leaf && tree.tree_type == BPLUS) {
		...
		memcpy(get_keys(right_node),
		       get_key_at(tree, node, split_index + 1),
		...
		if (tree.tree_type == BTREE) {
			...
			memcpy(dst_records,
			       src_records +
				   (split_index + 1) * tree.record_size,
			       right_node->num_keys * tree.record_size);
		}
...
insert(BTree &tree, BTreeNode *node, uint8_t *key, const uint8_t *data)
{
	if (node->is_leaf) {
		...
		memcpy(get_key_at(tree, node, insert_index), key,
		       tree.node_key_size);
...
insert_element(BTree &tree, void *key, const uint8_t *data)
{
	...
	if (root->num_keys == 0) {
		...
		if (!insert(tree, root, static_cast<uint8_t *>(key), data)) {
			insert(tree, get_root(tree),
			       static_cast<uint8_t *>(key), data);
		}
	}
}
...
do_delete_btree(BTree &tree, BTreeNode *node, const uint8_t *key, uint32_t i)
{
	if (node->is_leaf) {
		...
		mark_dirty(node);
		memcpy(get_key_at(tree, node, index), pred_key,
		       tree.node_key_size);
		uint8_t *internal_records =
		    get_internal_record_data(tree, node);
...
static void
do_delete_bplus(BTree &tree, BTreeNode *node, const uint8_t *key, uint32_t i)
{
	if (node->is_leaf) {
		mark_dirty(node);
...
steal_from_right(BTree &tree, BTreeNode *node, uint32_t parent_index)
{
	...
	if (node->is_leaf) {
		...
		uint8_t *sibling_records =
		    get_leaf_record_data(tree, right_sibling);

		memcpy(node_records + node->num_keys * tree.record_size,
		       sibling_records, tree.record_size);
		...
		memcpy(get_key_at(tree, node, node->num_keys),
		       get_key_at(tree, parent_node, parent_index),
		       tree.node_key_size);

		if (tree.tree_type == BTREE) {
			uint8_t *node_records =
			    get_internal_record_data(tree, node);
			uint8_t *parent_records =
			    get_internal_record_data(tree, parent_node);
			uint8_t *sibling_records =
			    get_internal_record_data(tree, right_sibling);
		...
		for (uint32_t i = 0; i < right_sibling->num_keys; i++) {
			set_child(tree, right_sibling, i,
				  sibling_children[i + 1]);
		}
	}
...
merge_right(BTree &tree, BTreeNode *node)
{
	...
	if (node->is_leaf) {
		...
		memcpy(get_key_at(tree, node, node->num_keys),
		       get_key_at(tree, parent, node_index),
		       tree.node_key_size);
		...
		if (tree.tree_type == BTREE) {
			...
			memcpy(node_records + node->num_keys * tree.record_size,
			       parent_records + node_index * tree.record_size,
			       tree.record_size);

			memcpy(node_records +
				   (node->num_keys + 1) * tree.record_size,
			       sibling_records,
			       right_sibling->num_keys * tree.record_size);
		}
		...
		for (uint32_t i = 0; i <= right_sibling->num_keys; i++) {
			set_child(tree, node, node->num_keys + 1 + i,
				  sibling_children[i]);
		}
	...
	memcpy(get_key_at(tree, parent, node_index),
	       ...
	       shift_count * tree.node_key_size);
	memcpy(parent_children + node_index + 1,
	       parent_children + node_index + 2,
	...
	if (node->is_leaf) {
		...
		if (tree.tree_type == BTREE) {
			...
			uint8_t *parent_records =
			    get_internal_record_data(tree, parent_node);
			uint8_t *sibling_records =
			    get_internal_record_data(tree, left_sibling);
			...
			memcpy(get_key_at(tree, parent_node, parent_index - 1),
			       get_key_at(tree, left_sibling,
					  left_sibling->num_keys - 1),
			       tree.node_key_size);
			memcpy(parent_records +
				   (parent_index - 1) * tree.record_size,
			       sibling_records + (left_sibling->num_keys - 1) *
						     tree.record_size,
			       tree.record_size);
		} else {
			memcpy(get_key_at(tree, parent_node, parent_index - 1),
		...
		set_child(tree, node, 0,
			  sibling_children[left_sibling->num_keys]);
	}
...
repair_after_delete(BTree &tree, BTreeNode *node)
{
	...
	if (node_index > 0) {
		BTreeNode *left_sibling =
		    get_child(tree, parent, node_index - 1);
		if (left_sibling &&
	...
	if (node_index < parent->num_keys) {
		BTreeNode *right_sibling =
		    get_child(tree, parent, node_index + 1);
		if (right_sibling && right_sibling->num_keys >
					 get_min_keys(tree, right_sibling)) {
...
// functions
static void
cursor_push_stack(BtCursor *cursor, uint32_t page, uint32_t index,
		  uint32_t child_pos)
{
...
static FindResult
find_containing_node(BTree &tree, BTreeNode *node, const uint8_t *key,
		     BtCursor *cursor)
{
...
cursor_move_in_subtree(BtCursor *cursor, BTreeNode *root, bool left)
{
	...
	while (!current->is_leaf) {
		if (left) {
			...
			cursor_push_stack(cursor, current->index,
					  current->num_keys - 1, child_pos);
...
btree_cursor_seek_cmp(BtCursor *cursor, const void *key, CompareOp op)
{
	...
	do {
		...
		if (satisfied)
			return true;
	} while (forward ? btree_cursor_next(cursor)
...
btree_cursor_seek(BtCursor *cursor, const void *key)
{
	...
	cursor->state = (result.index < result.node->num_keys) ? CURSOR_VALID
							       : CURSOR_INVALID;
	return false;
}
...
btree_cursor_next(BtCursor *cursor)
{
	...
	if (node->is_leaf) {
		...
		while (cursor->path.stack_depth > 0) {
			uint32_t child_pos =
			    cursor->path
				.child_stack[cursor->path.stack_depth - 1];
			...
			BTreeNode *parent = static_cast<BTreeNode *>(
			    pager_get(cursor->path.current_page));
		...
		if (child) {
			cursor_push_stack(cursor, node->index,
					  cursor->path.current_index,
					  next_child);
			return cursor_move_to_leftmost_in_subtree(cursor,
								  child);
		}
	}
...
btree_cursor_previous(BtCursor *cursor)
{
	...
	if (node->is_leaf) {
		...
		while (cursor->path.stack_depth > 0) {
			...
			// visit key i
			if (child_pos > 0 &&
			    child_pos == cursor->path.current_index + 1) {
				cursor->state = CURSOR_VALID;
				return true;
			}
			...
			if (cursor->path.current_index > 0) {
				...
				if (child) {
					cursor_push_stack(
					    ...
					    cursor->path.current_index,
					    cursor->path.current_index + 1);
					return cursor_move_to_rightmost_in_subtree(
					    cursor, child);
				}
			}
		}
		...
		if (child) {
			cursor_push_stack(cursor, node->index,
					  cursor->path.current_index,
					  cursor->path.current_index);
			return cursor_move_to_rightmost_in_subtree(cursor,
								   child);
		}
	}
            ...
            )
{
	...
	while (btree_cursor_is_valid(cursor)) {
		...
		if (cmp(cursor->tree->node_key_size, current_key,
			(uint8_t *)key) != 0) {
			break; // Passed all duplicates
		}

		uint8_t *current_record = btree_cursor_record(cursor);
		if (memcmp(current_record, record, cursor->tree->record_size) ==
		    0) {
