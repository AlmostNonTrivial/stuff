...
static uint32_t* get_children(BTreeNode* node) {
    ...
    return reinterpret_cast<uint32_t*>(node->data + node->max_keys * sizeof(uint32_t));
}
...
static uint8_t* get_record_at(BTreeNode* node, uint32_t index) {
    if (!node->is_leaf || index >= node->num_keys) return nullptr;
...
BPlusTreeCapacity bp_calculate_capacity(const std::vector<ColumnInfo> &schema) {
  ...
  // = n * 4 + (n+1) * 4 = n * 8 + 4
  uint32_t internal_max_keys = (USABLE_SPACE - sizeof(uint32_t)) / (2 * sizeof(uint32_t));
  ...
  return {
      std::min(leaf_max_keys, internal_max_keys),
...
BPlusTree bp_create(const std::vector<ColumnInfo>& schema) {
    ...
    tree.internal_max_keys = (USABLE_SPACE - sizeof(uint32_t)) / (2 * sizeof(uint32_t));
...
BTreeNode* bp_get_parent(BTreeNode* node) {
    if (!node || node->parent == 0) return nullptr;
...
BTreeNode* bp_split(BPlusTree& tree, BTreeNode* node) {
    ...
    if (node->parent != 0) {
        ...
        // Find parent index
        for (parent_index = 0;
             parent_index < current_parent->num_keys + 1 &&
    ...
    if (i == node->num_keys) {
        ...
        for (uint32_t j = i; j < node->num_keys - 1; j++) {
            ...
            memcpy(record_data + j * tree.record_size,
                   record_data + (j + 1) * tree.record_size,
                   tree.record_size);
        }
        node->num_keys--;
        bp_mark_dirty(node);
...
void bp_repair_after_delete(BPlusTree& tree, BTreeNode* node) {
    if (node->num_keys < bp_get_min_keys(tree, node)) {
        if (node->parent == 0) {
            ...
            BTreeNode* left_sibling = (parent_index > 0) ? bp_get_child(parent_node, parent_index - 1) : nullptr;
            BTreeNode* right_sibling = (parent_index < parent_node->num_keys) ? bp_get_child(parent_node, parent_index + 1) : nullptr;

            if (left_sibling && left_sibling->num_keys > bp_get_min_keys(tree, left_sibling)) {
                bp_steal_from_left(tree, node, parent_index);
            } else if (right_sibling && right_sibling->num_keys > bp_get_min_keys(tree, right_sibling)) {
    ...
    if (node->is_leaf) {
        ...
        for (uint32_t i = 1; i < right_sib->num_keys; i++) {
            ...
            memcpy(right_records + (i - 1) * tree.record_size,
                   right_records + i * tree.record_size,
                   tree.record_size);
        }
    } else {
    ...
    if (node->is_leaf) {
        ...
        for (uint32_t i = node->num_keys - 1; i > 0; i--) {
            memcpy(node_records + i * tree.record_size,
                   node_records + (i - 1) * tree.record_size,
                   tree.record_size);
        }
