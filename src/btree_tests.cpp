...
void test_internal_node_operations() {
    ...
    for (auto type : {BPLUS, BTREE}) {
        ...
        for (uint32_t i = 1; i <= tree.leaf_max_keys * tree.internal_max_keys; i++) {
            uint8_t record[TYPE_INT32];
            memcpy(record, &i, sizeof(i));
            bp_insert_element(tree, i, record);
        }
...
void test_boundary_conditions() {
    ...
    for (auto type : {BPLUS, BTREE}) {
        ...
        uint32_t test_counts[] = {
            tree.leaf_min_keys,
            tree.leaf_min_keys + 1,
            tree.leaf_max_keys - 1,
            tree.leaf_max_keys,
...
void verify_btree_specific_invariants(BPlusTree &tree) {
    if (tree.tree_type != BTREE) return;
    ...
    while (!to_visit.empty()) {
        ...
        if (!node->is_leaf) {
            ...
            for (uint32_t i = 0; i < node->num_keys; i++) {
                uint8_t *record = get_record_at(tree, node, i);
                if (!record) {
                    std::cerr << "B-tree internal node missing record for key " << i << std::endl;
                    exit(1);
                }
            }
...
void test_basic_persistence() {
    ...
    // Session 1: Insert data and commit
    {
        ...
        std::cout << "Session 1: Inserted 20 records, root page: " << saved_root_index << std::endl;
    ...
    // Session 2: Reopen and verify data persisted
    {
        ...
        std::vector<ColumnInfo> schema = {{TYPE_INT32}, {TYPE_VARCHAR32}};
        BPlusTree tree = bp_create(TYPE_INT32, schema, BPLUS);
        tree.root_page_index = saved_root_index; // In real system, this comes from catalog

        // Verify all records are still there
        bool all_found = true;
        for (int i = 0; i < 20; i++) {
            const TestRecord* result = reinterpret_cast<const TestRecord*>(bp_get(tree, i));
...
void test_transaction_rollback() {
    ...
    // Test: Make modifications then rollback
    {
        ...
        for (int i = 0; i < 10; i++) {
            const int32_t* result = reinterpret_cast<const int32_t*>(bp_get(tree, i));
...
BPlusTree tree ;
    ...
    // Session 2: Add more data
    {
        ...
        bool session1_visible = bp_find_element(tree, 5) && bp_find_element(tree, 14);
        check("Multi-session: Session 1 data visible in session 2", session1_visible);
    ...
    // Session 3: Verify all changes persisted
    {
        ...
        const TestRecord* orig = reinterpret_cast<const TestRecord*>(bp_get(tree, 10));
        bool orig_ok = orig && orig->id == 10 && strstr(orig->name, "Session1") != nullptr;
        check("Multi-session: Original data preserved", orig_ok);

        // Check updated data
        const TestRecord* updated = reinterpret_cast<const TestRecord*>(bp_get(tree, 2));
        bool update_ok = updated && updated->id == 1002 && strstr(updated->name, "Updated") != nullptr;
        check("Multi-session: Updates persisted", update_ok);

        // Check new data
        const TestRecord* new_data = reinterpret_cast<const TestRecord*>(bp_get(tree, 25));
        bool new_ok = new_data && new_data->id == 25 && strstr(new_data->name, "Session2") != nullptr;
...
void test_crash_recovery_simulation() {
    ...
    // Recovery: Reopen and verify uncommitted changes are gone
    {
        ...
        const int32_t* restored = reinterpret_cast<const int32_t*>(bp_get(tree, 2));
        bool crash_updates_gone = restored && *restored == 20; // Original value, not 2000
...
void test_large_transaction_rollback() {
...

    {
        ...
        pager_rollback();
        pager_close();
        std::cout << "Rolled back transaction with " << large_count << " inserts" << std::endl;
    }
...
void test_multi_tree_isolation() {
...

    {
        ...
        std::vector<ColumnInfo> users_schema = {{TYPE_INT32}, {TYPE_VARCHAR32}};
        std::vector<ColumnInfo> orders_schema = {{TYPE_INT32}, {TYPE_INT64}};
        std::vector<ColumnInfo> products_schema = {{TYPE_VARCHAR32}};
        ...
        for (int i = 0; i < 10; i++) {
            ...
            bp_insert_element(users_tree, i, reinterpret_cast<const uint8_t*>(&user));

            // Orders table
            struct { int32_t order_id; int64_t amount; } order = {i + 1000, i * 100LL};
            bp_insert_element(orders_tree, i, reinterpret_cast<const uint8_t*>(&order));

            // Products table
            char product[32];
            snprintf(product, sizeof(product), "Product_%d", i);
            bp_insert_element(products_tree, i, reinterpret_cast<const uint8_t*>(product));
        }
        ...
        const TestRecord* user = reinterpret_cast<const TestRecord*>(bp_get(users_tree, 3));
        bool user_data_correct = user && user->id == 3 && strstr(user->name, "User_3") != nullptr;
...
void test_multi_tree_transactions() {
    ...
    std::vector<ColumnInfo> table1_schema = {{TYPE_INT32}};
    std::vector<ColumnInfo> table2_schema = {{TYPE_INT32}};
    ...
    // Test: Modify both trees, then rollback
    {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            bp_insert_element(table1, i, reinterpret_cast<const uint8_t*>(&new_value1));
            bp_insert_element(table2, i, reinterpret_cast<const uint8_t*>(&new_value2));
        }

        // Verify changes are visible
        const int32_t* t1_new = reinterpret_cast<const int32_t*>(bp_get(table1, 12));
        const int32_t* t2_new = reinterpret_cast<const int32_t*>(bp_get(table2, 12));
        bool changes_visible = t1_new && *t1_new == 1200 && t2_new && *t2_new == 2400;
    ...
    // Verify: Both trees should be restored to original state
    {
        ...
        bool inserts_rolled_back = !bp_find_element(table1, 12) && !bp_find_element(table2, 12);
        check("Multi-tree txn: Inserts rolled back from both trees", inserts_rolled_back);

        // Check that updates were rolled back to original values
        const int32_t* t1_orig = reinterpret_cast<const int32_t*>(bp_get(table1, 2));
        const int32_t* t2_orig = reinterpret_cast<const int32_t*>(bp_get(table2, 2));
        bool updates_rolled_back = t1_orig && *t1_orig == 20 && t2_orig && *t2_orig == 40;
        check("Multi-tree txn: Updates rolled back to original values", updates_rolled_back);
...
void test_multi_tree_page_sharing() {
...

    {
        ...
        std::cout << "Tree roots: " << tree1.root_page_index << ", "
                 << tree2.root_page_index << ", " << tree3.root_page_index << std::endl;
        ...
        bool data_integrity = val1 && *val1 == 25 &&
                             val2 && *val2 == 1025 &&
                             val3 && *val3 == 2025;
...
void test_mixed_tree_types() {
...

    {
        ...
        for (int i = 0; i < 20; i++) {
            int32_t value = i * 50;
            bp_insert_element(bplus_tree, i, reinterpret_cast<const uint8_t*>(&value));
            bp_insert_element(btree_tree, i, reinterpret_cast<const uint8_t*>(&value));
        }

        // Insert duplicates in B-tree (should be allowed)
        for (int i = 0; i < 5; i++) {
            int32_t duplicate_value = i * 50 + 1; // Slightly different value
            bp_insert_element(btree_tree, i, reinterpret_cast<const uint8_t*>(&duplicate_value));
        }
        ...
        const int32_t* bplus_val = reinterpret_cast<const int32_t*>(bp_get(bplus_tree, 2));
...
void test_concurrent_tree_operations() {
...

    {
        ...
        for (int i = 0; i < 30; i++) {
            ...
            // Insert to log tree
            bp_insert_element(log_tree, i, reinterpret_cast<const uint8_t*>(&log_entry));

            // Every 3rd operation, insert to data tree
            if (i % 3 == 0) {
                bp_insert_element(data_tree, i/3, reinterpret_cast<const uint8_t*>(&data_value));
            }
...
void run_integration_tests() {
    ...
    std::cout << "Tested: persistence, rollback, multi-session, crash recovery, multi-tree support" << std::endl;
}
