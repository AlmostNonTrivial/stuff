...
bool bp_validate_tree(BPlusTree &tree) {
  ...
  if (tree.tree_type == BTREE) {
    ...
    uint32_t expected_internal_max = std::max(MIN_KEYS, (USABLE_SPACE - tree.node_key_size) /
...
bool test_single_leaf_operations_btree() {
  std::vector<ColumnInfo> schema = {{TYPE_INT32}, {TYPE_VARCHAR32}};
  uint8_t record[36];
  memset(record, 0, sizeof(record));

  BPlusTree tree = bp_create(TYPE_INT32, schema, BTREE);
  bp_init(tree);
  uint32_t expected_count = 0;

  for (uint32_t i = 0; i < tree.leaf_max_keys; i++) {
    bp_insert_element(tree, i * 10, record);
    expected_count++;
    BPTreeNode* root = bp_get_root(tree);
    if (!root || !root->is_leaf || root->num_keys != expected_count) {
      std::cerr << "BTREE Test failed: Invalid root state after insert key " << i * 10 << std::endl;
      return false;
    }
    if (!bp_find_element(tree, i * 10)) {
      std::cerr << "BTREE Test failed: Key " << i * 10 << " not found after insert" << std::endl;
      return false;
    }
    if (i >= 3 && i % 3 == 0) {
      uint32_t delete_key = (i - 1) * 10;
      bp_delete_element(tree, delete_key);
      expected_count--;
      if (bp_find_element(tree, delete_key)) {
        std::cerr << "BTREE Test failed: Deleted key " << delete_key << " still found" << std::endl;
        return false;
      }
      root = bp_get_root(tree);
      if (!root || root->num_keys != expected_count) {
        std::cerr << "BTREE Test failed: Invalid root state after delete key " << delete_key << std::endl;
        return false;
      }
    }
  }
  ...
  for (uint32_t i = 0; i < tree.leaf_max_keys; i++) {
    bool should_exist = !(i >= 3 && (i - 1) % 3 == 0);
    if (bp_find_element(tree, i * 10) != should_exist) {
      std::cerr << "BTREE Test failed: Key " << i * 10 << " existence check failed (expected: "
                << (should_exist ? "found" : "not found") << ")" << std::endl;
      return false;
    }
  }
 ...
 */
#include <set>
bool test_single_leaf_operations_bplus() {
  ...
  for (uint32_t key : keys) {
    ...
    if (!root || !root->is_leaf || root->num_keys != inserted_keys.size()) {
      std::cerr << "B+TREE Test failed: Invalid root state after insert key " << key << std::endl;
      return false;
    }

    if (!bp_find_element(tree, key)) {
      std::cerr << "B+TREE Test failed: Key " << key << " not found after insert" << std::endl;
      return false;
    }
  }
  ...
  for (int i = 0; i < 5; i++) {
    ...
    BPTreeNode* root = bp_get_root(tree);
    validate_bplus_leaf_node(tree, root);
    if (root->num_keys != initial_count) {
  ...
  for (uint32_t delete_key : to_delete) {
    ...
    if (bp_find_element(tree, delete_key)) {
      std::cerr << "B+TREE Test failed: Deleted key " << delete_key << " still found" << std::endl;
      return false;
    }
    ...
    if (!root || root->num_keys != inserted_keys.size()) {
      std::cerr << "B+TREE Test failed: Invalid root state after delete key " << delete_key << std::endl;
      return false;
    }
  }
  ...
  // Validate all keys are in order and values correspond to keys
  BPTreeNode* root = bp_get_root(tree);
  if (!root || root->num_keys != tree.leaf_max_keys) {
    std::cerr << "B+TREE Test failed: Tree not at maximum capacity" << std::endl;
    return false;
  }
  ...
  if (leaf_data.size() != tree.leaf_max_keys) {
    std::cerr << "B+TREE Test failed: Expected " << tree.leaf_max_keys
  ...
  for (size_t i = 1; i < leaf_data.size(); i++) {
    if (leaf_data[i-1].first >= leaf_data[i].first) {
      std::cerr << "B+TREE Test failed: Keys not in order at positions "
                << (i-1) << " and " << i << " (values "
                << leaf_data[i-1].first << ", " << leaf_data[i].first << ")" << std::endl;
      return false;
    }
  }
...
bool test_single_leaf_operations() {
  // bool btree_result = test_single_leaf_operations_btree();
  bool bplus_result = test_single_leaf_operations_bplus();
  // return btree_result && bplus_result;
  return bplus_result;
}
