...
struct is_arena_tag : std::false_type {};

template<typename T>
struct is_arena_tag<T, std::void_t<decltype(Arena<T>::alloc(0))>> : std::true_type {};
...
class Vec {
    static_assert(is_arena_tag<ArenaTag>::value, "Second parameter must be an Arena tag");
...
public:
    // Constructor
    Vec() : data(nullptr), capacity(0), count(0), head(0), tail(0), is_queue_mode(false) {}
    ...
    void reserve(size_t new_capacity) {
        ...
        if (data && new_data) {
            if (is_queue_mode && head != 0) {
                ...
                memcpy(new_data, data + head, first_part * sizeof(T));
                if (tail > 0) {
                    memcpy(new_data + first_part, data, tail * sizeof(T));
                }
                head = 0;
                tail = count;
            } else {
    ...
    void enable_queue_mode() {
        if (is_queue_mode) return;
        is_queue_mode = true;
        head = 0;
        tail = count;
    }
    ...
    void push_front(const T &item) {
        if (!is_queue_mode) enable_queue_mode();
    ...
    template<typename EqualFn>
    void insert_unique_with(const T &item, EqualFn eq) {
        auto pred = [&item, &eq](const T& elem) { return eq(elem, item); };
    ...
    template<typename U = T>
    typename std::enable_if<has_less_than<U>::value && has_equality<U>::value, int>::type
    ...
    int binary_search_with(const T &value, CompareFn cmp) const {
        int left = 0, right = count - 1;
        while (left <= right) {
            int mid = left + (right - left) / 2;
    ...
    insert_sorted(const T &item) {
        size_t pos = 0;
        while (pos < count && (*this)[pos] < item) pos++;
        insert(pos, item);
    }

    // Insert sorted with comparator
    template<typename CompareFn>
    void insert_sorted_with(const T &item, CompareFn cmp) {
        size_t pos = 0;
        while (pos < count && cmp((*this)[pos], item) < 0) pos++;
        insert(pos, item);
    }
    ...
    template<typename EqualFn>
    void deduplicate_with(EqualFn eq) {
        if (count <= 1) return;
    ...
    void normalize() {
        if (!is_queue_mode || head == 0) return;
...
private:
    ...
    int partition_op(int left, int right) {
        T pivot = (*this)[right];
        int i = left - 1;
    ...
    template<typename CompareFn>
    void quicksort_with(int left, int right, CompareFn cmp) {
        if (left >= right) return;
    ...
    template<typename CompareFn>
    int partition_with(int left, int right, CompareFn cmp) {
        T pivot = (*this)[right];
        int i = left - 1;
...
public:
    Vec() : capacity(StackSize), count(0), head(0), tail(0), is_queue_mode(false) {}

    // Basic operations
    void reserve(size_t new_capacity) {
        if (new_capacity > StackSize) {
            fprintf(stderr, "Stack vector overflow! Required: %zu, Available: %zu\n",
