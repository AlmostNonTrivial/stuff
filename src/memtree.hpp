...
static MemTreeNode* memtree_find_node(MemTree* tree, const uint8_t* key) {
    ...
    while (current) {
        ...
        if (cmp_result == 0) {
            return current;  // Returns first match for duplicates
...
static MemTreeNode* memtree_find_exact_node(MemTree* tree, const uint8_t* key,
                                            const uint8_t* record, NodeStack* stack) {
    stack->clear();
    MemTreeNode* current = tree->root;
...
inline bool memtree_delete(MemTree* tree, const uint8_t* key) {
    ...
    stack.clear();
    MemTreeNode* current = tree->root;
    MemTreeNode* node_to_delete = nullptr;
    NodeStack delete_stack;
    ...
    MemTreeNode* parent = delete_stack.top();
    MemTreeNode* node = node_to_delete;
...
inline bool memtree_delete_exact(MemTree* tree, const uint8_t* key, const uint8_t* record) {
    NodeStack stack;
    MemTreeNode* node = memtree_find_exact_node(tree, key, record, &stack);
    ...
    // Standard BST deletion (same as above)
    // Case 1: Node with no children
    if (!node->left && !node->right) {
        if (parent) {
            if (parent->left == node) {
...
inline bool memcursor_seek_exact(MemCursor* cursor, const void* key, const void* record) {
    cursor->stack.clear();
    MemTreeNode* current = cursor->tree->root;
...
// Check if there are duplicate keys at current position
inline bool memcursor_has_duplicates(MemCursor* cursor) {
    ...
    NodeStack saved_stack = cursor->stack;
    ...
    cursor->stack = saved_stack;
    cursor->state = MemCursor::VALID;

    return has_dup;
}

// Count duplicates with the same key
inline uint32_t memcursor_count_duplicates(MemCursor* cursor, const void* key) {
    if (!cursor->tree->allow_duplicates) {
