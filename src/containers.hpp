...
template <typename T, typename ArenaTag = global_arena,
          uint32_t InitialSize = 8>
struct array {

  contiguous<T, ArenaTag, InitialSize> storage;

  void reserve(uint32_t min_capacity) { storage.reserve(min_capacity); }
  ...
  void resize(uint32_t new_size) { storage.resize(new_size, true); }
  void shrink_to_fit() { storage.shrink_to_fit(); }

  template <typename OtherTag> void copy_from(const array<T, OtherTag> &other) {
  ...
  T *front() { return storage.size > 0 ? &storage.data[0] : nullptr; }
  const T *front() const {
  ...
  const T *end() const { return storage.data + storage.size; }
...
template <typename T, typename ArenaTag = global_arena> struct queue {
  ...
  void grow_to(uint32_t new_capacity) {
  ...
  T *front() { return count > 0 ? &storage.data[head] : nullptr; }

  const T *front() const { return count > 0 ? &storage.data[head] : nullptr; }
...
struct hash_map {
  struct Entry {
    ...
    V value;
    uint32_t hash;
    enum uint8_t { EMPTY = 0, OCCUPIED = 1, DELETED = 2 } state;
  };
  ...
  bool keys_equal(const K &stored_key, const K &search_key) const {
  ...
  void grow() {
    ...
    for (uint32_t i = 0; i < old_capacity; i++) {
      if (old_entries[i].state == Entry::OCCUPIED) {
        ...
        insert_into(new_storage.data, new_storage.capacity, old_entries[i].key,
                    old_entries[i].hash, old_entries[i].value);
      }
    }
  ...
  V *get(const K &key) {
    ...
    while (true) {
      ...
      if (entry.state == Entry::OCCUPIED && entry.hash == hash &&
  ...
  uint32_t capacity() const { return storage.capacity; }
  Entry *entries() const { return storage.data; }
  uint32_t size() const { return _size; }
  uint32_t tombstone_count() const { return tombstones; }

  Entry *data() { return storage.data; }
  const Entry *data() const { return storage.data; }

private:
  // Core insertion logic - no growth checks, works on any buffer
  V *insert_into(Entry *entries, uint32_t capacity, const K &key, uint32_t hash,
                 const V &value, bool handle_tombstones = false) {
    ...
    while (true) {
      ...
      if (entry.state == Entry::EMPTY ||
          (!handle_tombstones && entry.state != Entry::OCCUPIED)) {
        Entry *target = (handle_tombstones && first_deleted != -1)
                            ? &entries[first_deleted]
                            : &entry;
      ...
      if (handle_tombstones && entry.state == Entry::DELETED &&
          first_deleted == -1) {
        first_deleted = idx;
      } else if (entry.state == Entry::OCCUPIED && entry.hash == hash &&
                 keys_equal(entry.key, key)) {
        entry.value = value;
        return &entry.value;
      }
  ...
  struct iterator {
    ...
    // Constructor
    iterator(Entry *e, uint32_t cap, uint32_t idx)
        : entries(e), capacity(cap), index(idx) {
