...
struct BTreeValidator {
    ...
    NodeInfo validate_node(uint32_t page_index, uint32_t expected_parent,
                           uint32_t depth, uint32_t min_bound, uint32_t max_bound,
        ...
        if (node->parent != expected_parent) {
            errors.push_back("Parent mismatch at page " + std::to_string(page_index) +
                           ": expected " + std::to_string(expected_parent) +
                           ", got " + std::to_string(node->parent));
        }

        // Check occupancy constraints (except root)
        if (expected_parent != 0) {
            if (node->num_keys < tree.min_keys) {
                errors.push_back("Underflow at page " + std::to_string(page_index) +
                               ": has " + std::to_string(node->num_keys) +
                               " keys, min is " + std::to_string(tree.min_keys));
            }
        }

        if (node->num_keys > tree.max_keys) {
            errors.push_back("Overflow at page " + std::to_string(page_index) +
                           ": has " + std::to_string(node->num_keys) +
                           " keys, max is " + std::to_string(tree.max_keys));
        }
        ...
        for (uint32_t i = 0; i < node->num_keys; i++) {
            uint32_t* key_ptr = reinterpret_cast<uint32_t*>(
                node->data + i * tree.node_key_size);
            ...
            if (has_prev && key < prev_key) {
                errors.push_back("Keys not sorted at page " + std::to_string(page_index) +
                               ", position " + std::to_string(i));
            }

            // Check bounds
            if (check_min && key < min_bound) {
                errors.push_back("Key " + std::to_string(key) + " violates min bound " +
                               std::to_string(min_bound) + " at page " + std::to_string(page_index));
            }
            if (check_max && key >= max_bound) {
                errors.push_back("Key " + std::to_string(key) + " violates max bound " +
                               std::to_string(max_bound) + " at page " + std::to_string(page_index));
            }
        ...
        if (!node->is_leaf) {
            uint32_t* children = reinterpret_cast<uint32_t*>(
                node->data + tree.max_keys * tree.node_key_size +
                tree.max_keys * tree.record_size);

            for (uint32_t i = 0; i <= node->num_keys; i++) {
                if (children[i] == 0) {
                    errors.push_back("Null child pointer at page " + std::to_string(page_index) +
                                   ", position " + std::to_string(i));
                    continue;
                }

                // Determine bounds for this child
                uint32_t child_min = (i == 0) ? min_bound :
                    *reinterpret_cast<uint32_t*>(node->data + (i-1) * tree.node_key_size);
                uint32_t child_max = (i == node->num_keys) ? max_bound :
                ...
                NodeInfo child_info = validate_node(children[i], page_index, depth + 1,
                                                   child_min, child_max,
                ...
                if (i == 0) {
                    ...
                    errors.push_back("Inconsistent child depths at page " +
        ...
        return {
            node->is_leaf ? depth : child_depth,
            actual_min,
            actual_max,
            has_actual_min,
            has_actual_max
...
inline void test_btree_stress() {
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        if (op < 60 || reference_counts.empty()) {
            ...
            if (i % 100 == 0) {
                uint32_t total = 0;
                for (auto& p : reference_counts) total += p.second;
                std::cout << "After insert " << i << ": tree has " << total
                         << " total entries, " << reference_counts.size() << " unique keys\n";
            }
            ...
            if (!reference_counts.empty()) {
                ...
                if (i % 100 == 0) {
                    uint32_t total = 0;
                    for (auto& p : reference_counts) total += p.second;
                    std::cout << "After delete " << i << ": tree has " << total
                             << " total entries, " << reference_counts.size() << " unique keys\n";
                }
            }
    ...
    for (auto& p : reference_counts) expected_total += p.second;
    ...
    for (auto& [key, count] : reference_counts) {
        if (tree_counts[key] != count) {
            std::cout << "ERROR: Key " << key << " has " << tree_counts[key]
                     << " instances, expected " << count << "\n";
            assert(false);
        }
    }
    ...
    std::cout << "Final tree has " << total_entries << " total entries with "
