...
void
update_table_stats(const char *table_name, const TableStats &stats)
{
	Table *table = get_table(table_name);
	if (table)
	{
...
void
update_index_stats(const char *table_name, uint32_t column_index, const IndexStats &stats)
{
	...
	if (index)
	{
		...
		if (_debug)
		{
			printf("Updated stats for index on '%s.%u': %u distinct keys, selectivity %.2f\n", table_name, column_index,
				   stats.distinct_keys, stats.selectivity);
		}
	}
}

void
invalidate_table_stats(const char *table_name)
{
	Table *table = get_table(table_name);
	if (table)
	{
...
TableStats *
get_table_stats(const char *table_name)
{
	Table *table = get_table(table_name);
...
IndexStats *
get_index_stats(const char *table_name, uint32_t column_index)
{
	Index *index = get_index(table_name, column_index);
...
add_table(Table *table)
{
	...
	if (_debug)
	{
		printf("Added table '%s' with %zu columns, record size %u bytes\n", table->table_name.c_str(),
			   table->columns.size(), layout.record_size);
	}
...
add_index(const char *table_name, Index *index)
{
	...
	if (_debug)
	{
		printf("Added index '%s' on column '%s' of table '%s' (needs ANALYZE)\n", index->index_name.c_str(),
			   table->columns[index->column_index].name.c_str(), table_name);
	}
...
void
print_record(uint8_t *record, const RecordLayout *layout)
{
	for (size_t i = 1; i < layout->column_count(); i++)
	{ // Skip key at index 0
		printf("[%zu]: ", i);
...
void
print_record_with_names(uint8_t *key, uint8_t *record, const Table *table)
{
	// Print key
...
void
print_table_info(const char *table_name)
{
	Table *table = get_table(table_name);
	if (!table)
	{
	...
	for (size_t i = 0; i < table->columns.size(); i++)
	{
		...
		case TYPE_32:
			...
			break;
		case TYPE_256:
			type_name = "VARCHAR256";
			break;
		default:
			type_name = "UNKNOWN";
			break;
		}
		printf("  %2zu: %-20s %s%s (offset: %u)\n", i, table->columns[i].name.c_str(), type_name, key_marker,
			   layout.get_offset(i));
	}
...
size_t
get_table_size(const char *table_name)
{
	Table *table = get_table(table_name);
	if (!table)
		return 0;
...
RecordLayout
build_layout_from_columns(const char *table_name, const Vec<const char *, QueryArena> &column_names)
{
	Table *table = get_table(table_name);
	if (!table)
	{
...
bool
column_exists_anywhere(const char *col_name)
{
	for (size_t i = 0; i < tables.size(); i++)
	{
...
bool
stats_are_fresh(const char *table_name)
{
	Table *table = get_table(table_name);
	if (!table)
		return false;
