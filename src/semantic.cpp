...
static void apply_catalog_changes() {
    ...
    // Add new tables
    for (auto entry : catalog_changes.tables_to_create) {
        catalog.insert(entry.key, entry.value);
    }
}

// Clear without applying
static void clear_catalog_changes() {
...
static Structure* lookup_table(const string<parser_arena>& name) {
...
static bool semantic_resolve_expr(Expr* expr, Structure* table) {
    if (!expr) return true;
...
static bool semantic_resolve_select(SelectStmt* stmt, const char** error, const char** context) {
    ...
    if (stmt->where_clause) {
        ...
        if (stmt->where_clause->sem.resolved_type != TYPE_U32 &&
            stmt->where_clause->sem.resolved_type != TYPE_NULL) {
...
static bool semantic_resolve_insert(InsertStmt* stmt, const char** error, const char** context) {
    // 1. Validate table exists
    Structure* table = lookup_table(stmt->table_name);
    if (!table) {
        *error = "Table does not exist";
    ...
    if (stmt->values.size != stmt->sem.column_indices.size) {
        *error = format_error("Value count mismatch: expected %u, got %u",
                             stmt->sem.column_indices.size, stmt->values.size);
        *context = stmt->table_name.c_str();
        return false;
    }
        ...
        if (expr->type == EXPR_LITERAL) {
            ...
            if (expr->lit_type != expected_type) {
                *error = format_error("Type mismatch for column '%s': expected %s, got %s",
                                     ...
                                     expected_type == TYPE_U32 ? "INT" : "TEXT",
...
static bool semantic_resolve_update(UpdateStmt* stmt, const char** error, const char** context) {
    // 1. Validate table exists
    Structure* table = lookup_table(stmt->table_name);
    if (!table) {
        *error = "Table does not exist";
    ...
    for (uint32_t i = 0; i < stmt->values.size; i++) {
        ...
        if (expr->type == EXPR_LITERAL) {
            ...
            if (expr->lit_type != expected_type) {
                *error = format_error("Type mismatch for column '%s'",
                                     table->columns[col_idx].name);
                *context = table->columns[col_idx].name;
                return false;
            }
...
static bool semantic_resolve_delete(DeleteStmt* stmt, const char** error, const char** context) {
    // 1. Validate table exists
    Structure* table = lookup_table(stmt->table_name);
    if (!table) {
        *error = "Table does not exist";
...
static bool semantic_resolve_create_table(CreateTableStmt* stmt, const char** error, const char** context) {
...
static bool semantic_resolve_drop_table(DropTableStmt* stmt, const char** error, const char** context) {
    Structure* table = lookup_table(stmt->table_name);
    if (!table) {
        *error = "Table does not exist";
        *context = stmt->table_name.c_str();
...
static bool semantic_resolve_statement(Statement* stmt, const char** error, const char** context) {
    switch (stmt->type) {
        case STMT_SELECT:
